...
impl<'a> DbMigrate<'a> {
    ...
    pub fn init_or_check<P: AsRef<Path>>(&self, fiber_dir: P) -> Result<&Store, String> {
        if self.need_init() {
            ...
            match self.check() {
                ...
                Ordering::Less => {
                    ...
                    return Err(format!("Fiber need to run some database migrations, please run `fnn-migrate -d {}` to start migrations.", fiber_dir.as_ref().display()));
                }
            }
        }
    }
