...
use crate::store::open_store_for_test;
...
fn test_store_watchtower() {
    ...
    let store = open_store_for_test(&path).expect("created store failed");
...
fn test_channel_actor_state_store() {
    ...
    let store = open_store_for_test(&path).expect("create store failed");
