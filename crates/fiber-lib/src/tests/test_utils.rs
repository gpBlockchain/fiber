...
pub fn generate_store() -> (Store, TempDir) {
    ...
    let store = open_store(temp_dir.as_ref(), temp_dir.as_ref());
    (store.expect("create store"), temp_dir)
}

#[cfg(target_arch = "wasm32")]
pub fn generate_store() -> (Store, ()) {
    let store = open_store(&PathBuf::default(), &PathBuf::default());
...
impl NetworkNodeConfigBuilder {
    ...
    pub fn build(self) -> NetworkNodeConfig {
        ...
        let store = open_store(&rand_db_dir, &rand_db_dir).expect("create store");
