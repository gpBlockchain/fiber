...
use crate::store::{open_store_for_test, Store};
use crate::{
...
pub fn generate_store() -> (Store, TempDir) {
    ...
    let store = open_store_for_test(temp_dir.as_ref());
    (store.expect("create store"), temp_dir)
}

#[cfg(target_arch = "wasm32")]
pub fn generate_store() -> (Store, ()) {
    let store = open_store_for_test(&PathBuf::default());
...
impl NetworkNodeConfigBuilder {
    ...
    pub fn build(self) -> NetworkNodeConfig {
        ...
        let store = open_store_for_test(&rand_db_dir).expect("create store");
